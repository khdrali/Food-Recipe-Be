...
const getRecipeByName=async(params)=>{
  const{type}=params
  return await db`SELECT * FROM recipe ${type?db`ORDER BY title DESC`:db`ORDER BY title ASC`}`
}
...
module.exports={getRecipeByName, getRecipePagination, getAllRecipes, createRecipe, recipeById, updateRecipe,deleteRecipe,searchRecipeName, searchRecipe}